...
Backend Worker (`Hono`) jest chroniony przez statyczny `USRC_API_KEY` w nagłówku `Authorization: Bearer <token>`. Jeśli ten klucz trafia do bundle'a frontendowego — każdy użytkownik może go odczytać z DevTools i wysyłać zapytania bezpośrednio do backendu.
...
Astro SSR (CF Worker)
    │
    │  Authorization: Bearer <USRC_API_KEY>   ← sekret z CF Secrets
    │  X-Appwrite-JWT: <token>                ← jeśli user zalogowany
    ▼
Backend Hono Worker
    │
    ├── ścieżka A: Appwrite JWT → weryfikacja przez Appwrite SDK → ok
    └── ścieżka B: USRC_API_KEY → statyczny sekret → ok (anonimowe/server-to-server)
```

**Zasada:** Przeglądarka nigdy nie zna `USRC_API_KEY`. Zna tylko swój własny JWT (który jest tokenem usera, nie sekretem serwera).
...
wrangler secret put USRC_API_KEY
...
export async function authMiddleware(c: Context, next: Next) {
  ...
  if (bearer && bearer === c.env.USRC_API_KEY) {
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  const headers: Record<string, string> = {
    ...
    'Authorization': `Bearer ${import.meta.env.USRC_API_KEY}`,
  }
...
# USRC_API_KEY → wrangler secret put USRC_API_KEY (nie tutaj!)
```
...
| Warstwa | USRC_API_KEY | Appwrite JWT | Appwrite projectId |
|---|---|---|---|
...
|---|---|---|
| Inicjacja uploadu do R2 | `USRC_API_KEY` (anonim ok) | Astro proxy → Backend |
...
| Operacje administracyjne | `USRC_API_KEY` (tylko server) | Backend → Backend / CF Cron |

---

## Checklist wdrożenia

- [ ] `wrangler secret put USRC_API_KEY` w projekcie frontend workera
- [ ] `wrangler secret put USRC_API_KEY` w projekcie backend workera  
