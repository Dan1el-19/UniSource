...
admin.get('/service', async (c) => {
  ...
  const response = c.json({ item: serviceData });
...
admin.get('/service/usage', async (c) => {
  ...
  const response = c.json({ item: data });
...
admin.get(
  ...
  async (c) => {
    ...
    try {
      ...
      const response = c.json({ items: result.items, page: { limit: query.limit, next_cursor: result.next_cursor } });
...
admin.get('/users', zValidator('query', userListQuerySchema, v2ValidationHook), async (c) => {
  ...
  const response = c.json({ items: users, page: { limit: query.limit, next_cursor: null }, total: appwriteUsers.total, offset: query.offset });
...
admin.post(
  ...
  async (c) => {
    ...
    const response = c.json({ item: { user_id: userId, password_reset: true } });
...
admin.post('/quota/reconcile', async (c) => {
  ...
  const response = c.json({ item: result as unknown as Record<string, unknown> });
