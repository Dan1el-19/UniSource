...
myFiles.get('/', zValidator('query', listQuerySchema, v2ValidationHook), async (c) => {
  ...
  try {
    ...
    const response = c.json({ items: result.items.map(mapFileRecord), page: { limit: query.limit, next_cursor: result.next_cursor } });
...
myFiles.get('/:id', zValidator('param', fileIdParamSchema, v2ValidationHook), async (c) => {
  ...
  const response = c.json({ item: file });
...
myFiles.delete('/:id', zValidator('param', fileIdParamSchema, v2ValidationHook), async (c) => {
  ...
  if (permanent) {
    ...
    const response = c.json({ item: { id, deleted: true, permanent: true } });
...
myFiles.patch(
  ...
  async (c) => {
    ...
    const response = c.json({ item: { id, success: true, folder_id: folder_id ?? null } });
