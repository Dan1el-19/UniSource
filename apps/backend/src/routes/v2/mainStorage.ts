...
mainStorage.get('/', zValidator('query', listQuerySchema, v2ValidationHook), async (c) => {
  ...
  try {
    ...
    const response = c.json({ items: result.items.map(toMainStorageFileResponse), page: { limit: limit, next_cursor: result.next_cursor } });
...
mainStorage.delete('/:id', async (c) => {
  ...
  const response = c.json({ item: { file_id: file.id, deleted: true } });
...
mainStorage.post('/:id/restore', async (c) => {
  ...
  const response = c.json({ item: { file_id: file.id, restored: true } });
