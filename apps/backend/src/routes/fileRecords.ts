...
myFiles.get('/', zValidator('query', listQuerySchema, validationErrorHook), async (c) => {
  ...
  try {
    const result = await listFileRecords(c.env.usrc_d1, {
      user_id: userId,
...
myFiles.get('/:id', zValidator('param', fileIdParamSchema, validationErrorHook), async (c) => {
  ...
  const record = await getFileRecordForUser(c.env.usrc_d1, id, userId);
  if (!record) {
...
myFiles.delete('/:id', zValidator('param', fileIdParamSchema, validationErrorHook), async (c) => {
  ...
  if (permanent) {
    ...
    await deleteFileRecordPermanently(c.env.usrc_d1, id, userId);
    return c.json({ success: true, id, permanent: true });
  }

  const trashed = await trashFileRecord(c.env.usrc_d1, id, userId);
  if (!trashed) {
...
myFiles.post('/:id/restore', zValidator('param', fileIdParamSchema, validationErrorHook), async (c) => {
  ...
  const restored = await restoreFileRecord(c.env.usrc_d1, id, userId);
  if (!restored) {
...
myFiles.patch(
  ...
  async (c) => {
    ...
    const moved = await moveFileRecord(c.env.usrc_d1, id, userId, folder_id ?? null);
    if (!moved) {
