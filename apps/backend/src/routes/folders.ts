...
folders.post('/', zValidator('json', folderCreateRequestSchema, validationErrorHook), async (c) => {
  ...
  if (body.parent_id) {
    const parent = await getFolderForUser(c.env.usrc_d1, body.parent_id, userId);
    if (!parent) {
...
folders.get('/', async (c) => {
  ...
  const items = await listFolders(c.env.usrc_d1, userId, parentId ?? undefined, trashedOnly);
...
folders.get('/:id', zValidator('param', folderIdParamSchema, validationErrorHook), async (c) => {
  ...
  const folder = await getFolderForUser(c.env.usrc_d1, id, userId);
  if (!folder) {
...
folders.patch(
  ...
  async (c) => {
    ...
    const updated = await updateFolder(c.env.usrc_d1, id, userId, {
      name: body.name,
...
folders.delete('/:id', zValidator('param', folderIdParamSchema, validationErrorHook), async (c) => {
  ...
  if (permanent) {
    const deleted = await deleteFolderPermanently(c.env.usrc_d1, id, userId);
    if (!deleted) {
      return c.json({ error: 'Not Found', message: 'Folder not found' }, 404);
    }
  ...
  const trashed = await trashFolder(c.env.usrc_d1, id, userId);
  if (!trashed) {
    return c.json({ error: 'Not Found', message: 'Folder not found or already trashed' }, 404);
  }
...
folders.post('/:id/restore', zValidator('param', folderIdParamSchema, validationErrorHook), async (c) => {
  ...
  const restored = await restoreFolder(c.env.usrc_d1, id, userId);
  if (!restored) {
