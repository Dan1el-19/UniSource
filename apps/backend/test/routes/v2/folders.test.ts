...
// ---------------------------------------------------------------------------
describe('POST /v2/folders/bulk-move', () => {
  beforeAll(async () => {
    await applyD1Migrations(env.APP_DB, env.TEST_MIGRATIONS)
  }, TEST_TIMEOUT)
  ...
  it('moves folders to a new parent and returns processed_count', async () => {
    await seedFolders([
      { id: 'target', user_id: USER_ID, service_id: SERVICE_ID, parent_id: null,
        name: 'target', color_tag: null, is_trashed: 0, trashed_at: null,
        created_at: 100, updated_at: 100 },
      { id: 'f1', user_id: USER_ID, service_id: SERVICE_ID, parent_id: null,
        name: 'a', color_tag: null, is_trashed: 0, trashed_at: null,
        created_at: 200, updated_at: 200 },
    ...
    const app = buildApp()
    const res = await app.fetch(
      new Request('http://localhost/v2/folders/bulk-move', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: ['f1'], parent_id: 'target' }),
      }),
      testEnv
    )
  ...
  it('returns 404 not_found when target parent_id does not exist', async () => {
    await seedFolders([
      { id: 'f1', user_id: USER_ID, service_id: SERVICE_ID, parent_id: null,
        name: 'a', color_tag: null, is_trashed: 0, trashed_at: null,
        created_at: 100, updated_at: 100 },
    ...
    const res = await app.fetch(
      new Request('http://localhost/v2/folders/bulk-move', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: ['f1'], parent_id: 'missing' }),
      }),
      testEnv
    )
    ...
    const body = await res.json() as any
    expect(body.error.code).toBe('not_found')
    expect(body.error.message).toBe('Target folder not found')
    expect(body.error.request_id).toBeTruthy()
  }, TEST_TIMEOUT)

  it('returns 409 validation_error when target is trashed', async () => {
    await seedFolders([
      { id: 'target', user_id: USER_ID, service_id: SERVICE_ID, parent_id: null,
        name: 'target', color_tag: null, is_trashed: 1, trashed_at: 50,
        created_at: 100, updated_at: 100 },
      { id: 'f1', user_id: USER_ID, service_id: SERVICE_ID, parent_id: null,
        name: 'a', color_tag: null, is_trashed: 0, trashed_at: null,
        created_at: 200, updated_at: 200 },
    ...
    const body = await res.json() as any
    expect(body.error.code).toBe('validation_error')
    expect(body.error.message).toMatch(/trashed/i)
  }, TEST_TIMEOUT)

  it('returns 409 validation_error when moving folder into itself', async () => {
    await seedFolders([
      { id: 'f1', user_id: USER_ID, service_id: SERVICE_ID, parent_id: null,
        name: 'a', color_tag: null, is_trashed: 0, trashed_at: null,
        created_at: 100, updated_at: 100 },
    ...
    const res = await app.fetch(
      new Request('http://localhost/v2/folders/bulk-move', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: ['f1'], parent_id: 'f1' }),
      }),
      testEnv
    )
  ...
  it('returns 409 validation_error on cycle (move parent into descendant)', async () => {
    ...
    await seedFolders([
      { id: 'root', user_id: USER_ID, service_id: SERVICE_ID, parent_id: null,
        ...
        created_at: 100, updated_at: 100 },
      { id: 'child', user_id: USER_ID, service_id: SERVICE_ID, parent_id: 'root',
        name: 'child', color_tag: null, is_trashed: 0, trashed_at: null,
        created_at: 200, updated_at: 200 },
      { id: 'grandchild', user_id: USER_ID, service_id: SERVICE_ID, parent_id: 'child',
        name: 'grandchild', color_tag: null, is_trashed: 0, trashed_at: null,
        created_at: 300, updated_at: 300 },
    ])

    // Try moving root INTO grandchild — cycle
    const app = buildApp()
    const res = await app.fetch(
      new Request('http://localhost/v2/folders/bulk-move', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: ['root'], parent_id: 'grandchild' }),
      }),
      testEnv
    )
    ...
    const body = await res.json() as any
    expect(body.error.code).toBe('validation_error')
    expect(body.error.message).toMatch(/cycle|descendant/i)
  }, TEST_TIMEOUT)
  ...
  it('returns validation_error with v2 shape on invalid body', async () => {
    ...
    const res = await app.fetch(
      new Request('http://localhost/v2/folders/bulk-move', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: [], parent_id: null }),
      }),
      testEnv
    )
