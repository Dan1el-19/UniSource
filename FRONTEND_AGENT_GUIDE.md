...
export const apiClient = new UnisourceClient({
  baseUrl: import.meta.env.PUBLIC_API_URL, // np. https://api.usrc.dev
  serviceId: import.meta.env.PUBLIC_SERVICE_ID, // np. "usrc" lub "blokserwis"
...
We wszystkich żądaniach musi być dołączony nagłówek `X-Service-ID` (co SDK narzuca przez propercję konfiguracyjną `serviceId` w konstruktorze). Izoluje to wszystkie dane pomiędzy np. stronillą UniSource a aplikacją Blokserwis.
...
Tylko przy użyciu `API Key` serwisu dla autoryzowanych paneli zarządzających (np. my.usrc.dev lub back-office dla najemców).
